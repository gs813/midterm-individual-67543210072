...
app.post('/api/students', (req, res) => {
    ...
    if (!student_code || !first_name || !last_name || !email || !major) {
        return res.status(400).json({ error: 'student_code, first_name, last_name, email, and major are required' });
    }

    const codePattern = /^\d{10}$/;
    if (!codePattern.test(student_code)) {
        return res.status(400).json({ error: 'Invalid student code format (must be 10 digits)' });
    }
    ...
    if (!validMajors.includes(major)) {
        return res.status(400).json({ error: 'Invalid major. Must be one of: CS, SE, IT, CE, DS' });
    }
...
app.patch('/api/students/:id/status', (req, res) => {
    ...
    if (!status || !validStatuses.includes(status)) {
        return res.status(400).json({ error: 'Invalid status. Must be one of: active, graduated, suspended, withdrawn' });
    }
    ...
    db.get('SELECT * FROM students WHERE id = ?', [id], (err, row) => {
        ...
        if (row.status === 'withdrawn') {
            return res.status(400).json({ error: 'Cannot change status of a withdrawn student' });
        }
